...
public class RegistrySaveManager {
    ...
    public void save(String name, boolean save_players) {
        try{
            ...
            String key_string = "barricades structures vehicles";
            if(save_players) {
            ...
            for(String key : keys) {
                ...
                for(String reg_key : reg_keys) {
                    data += "\"" + reg_key + "\"=\"" + WinRegistry.readString(WinRegistry.HKEY_CURRENT_USER,
    ...
    public void resetMap() {
        String[] keys = {"barricades", "structures", "vehicles", "inventory", "last", "clothes", "skills",
                "life", "position"};
        for(String key : keys) {
            List<String> reg_keys = getMatchingKeys(key);
            for(String reg_key : reg_keys) {
